...
function includesAny(text: string, keywords: readonly string[]): string[] {
  return keywords.filter((keyword) => {
    // For English keywords, use word boundaries. For Korean, simple includes.
...
function countLabLikeLines(text: string): number {
  return text
    .split(/\r?\n/)
    .filter((line) => /\b\d+(?:\.\d+)?\s*(mg\/dL|mmol\/L|mEq\/L|g\/dL|U\/L|%|x10|×10)\b/i.test(line) || /참고치|단위|결과값/.test(line))
    .length;
}
...
export function classifyChunk(chunk: ParsedChunk): ChunkClassification {
  ...
  if (imagingHits.length > 0 && (text.includes("findings") || text.includes("impression") || text.includes("판독"))) {
  ...
  if (medicationHits.length >= 2 && /\b\d+(?:\.\d+)?\s*(mg|mcg|g|ml|정|캡슐)\b/i.test(chunk.text)) {
