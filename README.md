...
AI 메뉴는 Gemini 3 Flash Preview를 사용합니다. GCS의 `uploads/` 파일 중 PDF, 이미지, DOCX, XLSX, PPTX, TXT, MD, CSV 파일과 `memos/`의 텍스트 메모를 선택해 분석합니다.  
...
현재 구현 기준으로 인증은 관리자 비밀번호 하나를 기준으로 동작합니다. 차기 Apple 스타일 프론트엔드/API 전환에서는 Cloudflare Access를 외부 접근 제어로 두고, 앱 내부 작업 보호는 패스키 인증으로 교체합니다.
...
- Cloudflare Access: `cloud.jisong.dev` 앞단에서 허용된 사용자/기기만 통과
- 패스키 인증: 앱 내부에서 WebAuthn 기반으로 민감 작업 보호
- 관리자 비밀번호: 패스키 전환 전까지의 임시 fallback
...
로컬에서 Cloudflare Access 헤더 없이 API를 확인해야 하면 임시로 `REQUIRE_CLOUDFLARE_ACCESS=false`를 설정합니다.
...
- `STORAGE_BACKEND=gcs`: Cloud Run 기본값
...
패스키 전환 전까지 `REQUIRE_AUTH_ALL=true`를 사용하면 도구모음까지 포함해 전체 앱이 관리자 비밀번호 확인 후 열립니다.
...
컨테이너 실행 시 `api_server:app`이 `0.0.0.0:8080`으로 열리도록 설정되어 있습니다. 이미지는 Cloud Build/Cloud Run 배포용으로 관리합니다.
...
- 환경변수: `GCS_BUCKET_NAME=jisong-cloud-storage`, `REQUIRE_CLOUDFLARE_ACCESS=true`, `CLOUDFLARE_ACCESS_ALLOWED_EMAILS=jsbang01357@gmail.com`, `ALLOW_GOOGLE_AUTH_FALLBACK=true`, `PASSKEY_RP_ID=cloud.jisong.dev`, `PASSKEY_ORIGIN=https://cloud.jisong.dev`, `PASSKEY_RP_NAME=Jisong Cloud`
