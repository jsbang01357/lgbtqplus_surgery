...
- 계정 ID fallback: 패스키가 어려운 환경에서 소유자 ID와 비밀번호로 앱 세션 발급
- Cloudflare Access: 사용할 수는 있지만 이메일 OTP UX를 피하기 위해 기본 필수 조건에서는 제외
...
- 시크릿: `GEMINI_API_KEY=gemini-api-key:latest`
- 계정 비밀번호는 GCS의 `auth/account_password.txt`에 저장하며, 설정 화면의 비밀번호 변경 기능으로 갱신합니다.
- 서비스 계정 지정 사용
...
- 현재 운영 API는 패스키 세션 또는 소유자 ID+비밀번호 세션을 확인하는 구조입니다. Cloudflare Access 헤더가 있으면 계속 인정하지만 기본 필수 조건은 아닙니다.
