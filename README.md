...
### 3. AI
- Gemini 기반 파일 분석
- Mac mini 병행 운영 시 Ollama 로컬 모델 분석 및 Gemini fallback
- 메모장 텍스트 분석
- 직접 입력 텍스트 분석
- SOAP 발표, 예상 Q&A 등 자주 쓰는 요청 프리셋
- 사용자 질문 기반 요약/정리
- 오늘/이번 달 예상 Gemini 비용을 원화로 표시
- 오늘 5,000원 또는 이번 달 15,000원 이상 사용 시 Gemini 분석 제한
- 분석 결과 복사, Markdown 다운로드, PDF 다운로드
- 분석 결과를 새 메모로 저장

AI 메뉴는 기본적으로 Gemini 3 Flash Preview를 사용하며, Mac mini 로컬 운영에서는 Ollama 모드로 `gemma4:e4b` 또는 `qwen3.5:9b`를 선택해 분석할 수 있습니다. `AI_PROVIDER=auto`에서는 Ollama를 먼저 시도하고 실패하면 Gemini로 fallback합니다. GCS의 `uploads/` 파일 중 PDF, 이미지, DOCX, XLSX, PPTX, TXT, MD, CSV 파일과 `memos/`의 텍스트 메모를 선택해 분석합니다.  
...
예상 비용 로그 기준으로 오늘 5,000원, 이번 달 15,000원에 도달하면 추가 Gemini 분석 버튼이 비활성화됩니다.  
개인 파일과 메모 저장 기능을 다루므로 웹하드/메모장과 동일하게 인증 후 접근합니다.
DOCX, XLSX, PPTX는 Gemini에 원본 파일을 직접 업로드하지 않고, 앱에서 텍스트를 추출해 분석 프롬프트에 포함합니다.
Ollama 모드는 텍스트 기반 분석만 지원하므로 PDF와 이미지는 원본 분석 대상에서 제외됩니다.
...
#### 저장소 상태
- 현재 storage backend 확인
- Mac mini 로컬 파일 수와 용량 확인
- GCS mirror 파일 수 확인
- 로컬 저장소와 GCS 수동 동기화
...
`mac.jisong.dev` Mac mini 병행 운영에서는 Cloudflare Access를 앱 앞단에 추가합니다. `cloud.jisong.dev`와 `mac.jisong.dev`는 같은 repo와 같은 GCS 버킷을 쓰되, 환경변수로 실행 위치와 AI provider를 나눕니다. Cloudflare Access는 외부 접근 제어, Streamlit 관리자 비밀번호는 앱 내부 작업 보호를 담당합니다.
...
### 저장소 backend
Mac mini 병행 운영에서는 로컬 디스크를 우선 쓰고 GCS와 mirror할 수 있습니다.

- `STORAGE_BACKEND=gcs`: 기존 Cloud Run 기본값
- `STORAGE_BACKEND=local`: 로컬 디스크만 사용
- `STORAGE_BACKEND=local_mirror`: 로컬 디스크를 우선 사용하고 GCS에 쓰기/삭제를 mirror
- `LOCAL_STORAGE_ROOT`: 컨테이너 내부 로컬 저장소 경로, Mac mini compose 기본값은 `/data/jisong-cloud`
- `LOCAL_STORAGE_SYNC_TO_GCS`: local mirror 쓰기 동기화 여부
- `LOCAL_STORAGE_PULL_FROM_GCS`: 목록 조회 시 GCS에서 로컬로 가져올지 여부
...
Mac mini 운영에서는 `REQUIRE_AUTH_ALL=true`를 권장합니다. 이 값이 켜져 있으면 도구모음까지 포함해 전체 앱이 관리자 비밀번호 확인 후 열립니다.
...
### AI provider 및 Ollama
Cloud Run과 Mac mini가 같은 코드로 동작하며, 아래 환경변수로 AI 경로를 나눕니다.

- `AI_PROVIDER`: `gemini`, `ollama`, `auto`
- `AI_FALLBACK_PROVIDER`: 기본값 `gemini`
- `OLLAMA_BASE_URL`: Mac mini Docker에서는 `http://host.docker.internal:11434`, Cloud Run에서 Mac mini tunnel을 호출할 때는 `https://ai.mac.jisong.dev`
- `OLLAMA_MODEL`: 기본값 `gemma4:e4b`
- `OLLAMA_MODEL_OPTIONS`: 기본값 `gemma4:e4b,qwen3.5:9b`
- `OLLAMA_CF_ACCESS_CLIENT_ID`
- `OLLAMA_CF_ACCESS_CLIENT_SECRET`

`cloud.jisong.dev`에서 Mac mini Ollama를 쓰려면 `ai.mac.jisong.dev`를 Cloudflare Tunnel과 Access service token 뒤에 두고, Cloud Run에 service token 값을 주입합니다.
...
Mac mini 병행 운영은 `docker-compose.local.yml`을 사용합니다.
