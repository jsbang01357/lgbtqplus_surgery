...
- [x] 텍스트 클리너 옵션 라벨 텍스트가 선택 상자 안에 안정적으로 들어가도록 레이아웃 보정
- [x] MD to PDF 도구에 저장소 Markdown 파일 선택/불러오기 기능 추가
- [x] JS 문법 검증과 diff whitespace 검증 실행

### 2026-05-26 파일/메모 CRUD 복구

- [x] 파일 업로드 실패 원인 확인 및 프론트 오류 처리 보강
- [x] 파일/메모 다운로드 실패 원인 확인 및 공통 다운로드 헤더 보강
- [x] 메모 ZIP/TXT 다운로드 API 회귀 테스트 추가
- [x] 메모 목록 카드에 다운로드/복사 액션 추가
- [x] 관련 테스트와 기본 검증 실행

## 이번 정리 요약

- 파일 업로드 API가 성공 후 존재하지 않는 `create_zip_of_files.clear()`를 호출해 500으로 끝나던 문제를 수정했다.
- 파일/메모 다운로드 응답에서 한글 파일명이 raw header로 들어가던 부분을 RFC 5987 `filename*` 방식으로 보강했다.
- 메모 ZIP API가 `/api/memos/{file_name}` 라우트에 먼저 잡히고, temp zip path를 잘못 스트리밍하던 문제를 `FileResponse`로 고쳤다.
- 메모 목록 카드에 `다운`, `복사` 액션을 추가했다.
- 파이썬 회귀 테스트 49개 통과, JS 문법 검증 통과를 확인했다.
